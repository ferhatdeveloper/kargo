# Navlun Portal — Stocado Parite TODO Listesi
...
| Metrik | Değer |
|--------|--------|
| Placeholder sayfa (Navlun) | **9** |
| Kısmi / iskelet sayfa | **7** |
| Hedef: P0 tamamlanınca “boş taslak” algısı | ~%70 azalır |

**Kısıtlar (ürün):**

- Giriş sayfası (`/auth/login`) Stocado ile bire bir yapılmayacak — Navlun split layout korunur.
- API katmanı: PostgREST + RPC (`portal/src/api/rpc.ts`).
- Marka: **Navlun** (Stocado adı UI’da yok).
